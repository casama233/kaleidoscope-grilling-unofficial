...
export function readCookeryOilPot(stack,{legacyPlacementFallback=false}={}){
 ...
 const count=normalizeOilCount({
  ...
  legacyPlacementFallback
 });
 return {filled:true,empty:false,type,count,capacity:oilCapacity(type)};
}

export function readCookeryOilPotForPlacement(stack){
 return readCookeryOilPot(stack,{legacyPlacementFallback:true});
}
...
export function planCookeryTypedOilAddition(stack,incomingType,points){
 const state=readCookeryOilPot(stack);
 const plan=planTypedOilAddition(state,incomingType,points);
 if(!plan.ok)return plan;
