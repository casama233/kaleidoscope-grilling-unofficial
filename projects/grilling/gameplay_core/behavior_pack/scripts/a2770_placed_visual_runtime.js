...
function dispose(row,slot){
 ...
 try{if(entry.entity.isValid)entry.entity.remove()}catch(error){warn(error)}
 owned.delete(entry.id);row.helpers.delete(slot);
}
...
function render(row,dimension,slot,type,location,rotation,plan){
 ...
 if(entry&&(!entry.entity.isValid||entry.type!==type)){dispose(row,slot);entry=undefined}
 if(!entry){
 ...
 try{
  ...
  if(type===OIL_ENTITY)entity.setProperty(PREFIX+'oil',plan.oil);
   ...
   for(let i=0;i<8;i++)for(let shade=0;shade<2;shade++)
    entity.setProperty(PREFIX+'color_'+(i*2+shade),(plan.colors[i]||FALLBACK_COLORS)[shade]);
  }
...
function sync(row,players){
 ...
 if(isSeasoningBlock(block.typeId)){
  ...
  clear(row);tracked.delete(visualLocationKey(row.dimensionId,l));
 }
}
function pump(){
