# A2.7.59 — Inventory / Dimension Advancement Parity
...
Bedrock 主 `main.js` 已經有一個遊戲主迴圈，因此 A2.7.59 只在既有 player loop 裡加入：
...
A2.7.59 只是補這個 Java fallback，仍使用同一 one-shot property；不會重複給 XP。
