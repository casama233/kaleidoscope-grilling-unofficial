...
## 8. A2.7.33 同步（2026-09-22 部署）

上游推進到 A2.7.33（含調料瓶修正、榨油壺轉接層、農田作物宿主整合、獨立食物效果統一）。伺服器版差異縮減為：

- 相依識別碼改接（同 #1）。
- `a2730_cookery_oil_pot_adapter.js`：上游新檔仍直接寫物品屬性，納入 itemData 改寫（`out`／`stack` 兩個變數）。
- 配方 unlock 改為通用推導（鐵錠 > 磚塊 > 空桶 > 首個材料），A2.7.33 新增的 `secret_chili_oil`（shapeless）也缺 unlock。
