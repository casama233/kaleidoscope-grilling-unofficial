...
t('host block contract stays exact',()=>{
 ...
 assert.match(TYPE_KEY_PREFIX,/^kaleidoscope_grilling:/);
});
...
t('host and addon persistence keys are coordinate stable',()=>{
 ...
 assert.equal(typedOilBlockKey('minecraft:overworld',1,64,-2),'kaleidoscope_grilling:a2735_oilpot_type_minecraft_overworld_p1_p64_m2');
});
...
console.log('A2.7.35 typed Cookery oil-pot block core: '+n+'/'+n);
