...
VERSION=[2,7,58]
CORE='a2758_pepper_worldgen_fruiting_core.js'
RUNTIME='a2758_pepper_worldgen_fruiting_runtime.js'
BLOCK='a2758_pepper_leaves_fruiting_bridge.block.json'
FEATURE='a2758_pepper_tree_worldgen.feature.json'
...
def patch_versions():
 ...
 for doc,name in (
  (bm,'Kaleidoscope Grilling A2.7.58 P1 Completion BP'),
  (rm,'Kaleidoscope Grilling A2.7.58 P1 Completion RP'),
 ):
 ...
 cfg['name']='Kaleidoscope Grilling A2.7.58 P1 Completion'
...
def patch_content():
 ...
 add=anchor+"import './a2758_pepper_worldgen_fruiting_runtime.js';\n"
 if s.count(anchor)!=1:raise RuntimeError('A2.7.58 Pepper runtime anchor drift')
 if 'a2758_pepper_worldgen_fruiting_runtime.js' in s:raise RuntimeError('A2.7.58 runtime already active')
 main.write_text(s.replace(anchor,add,1),encoding='utf-8')

def report():
 write(P/'reports/a2758-p1-completion.json',{
  'version':'A2.7.58',
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,57]:
  raise RuntimeError('A2.7.58 must augment published A2.7.57')
 patch_content();patch_versions();report()
 print('A2.7.58 P1 completion applied')
