...
def check_assets():
 ...
 for r in range(1,9):
  for v in range(8):
   ...
   assert not desc.get('animations') and not desc.get('scripts',{}).get('animate')
 ...
 for p in skewers:
  ...
  assert all(g.startswith('geometry.kg_a2764.') and g in models for g in desc['geometry'].values())
