...
def replace_once(s,old,new,label):
 if s.count(old)!=1:raise RuntimeError(f'A2.7.46 patch anchor drift ({label}): {s.count(old)}')
 return s.replace(old,new,1)
def git_blob(data):return hashlib.sha1(b'blob '+str(len(data)).encode()+b'\0'+data).hexdigest()

def patch_versions():
 bm,rm=load(BP/'manifest.json'),load(RP/'manifest.json')
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.46 Wedding Candy BP'),(rm,'Kaleidoscope Grilling A2.7.46 Wedding Candy RP')):
  doc['header']['version']=VERSION;doc['header']['name']=name
  for module in doc.get('modules',[]):module['version']=VERSION
 for dep in bm.get('dependencies',[]):
  if dep.get('uuid')==rm['header']['uuid']:dep['version']=VERSION
 write(BP/'manifest.json',bm);write(RP/'manifest.json',rm)
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.46 Wedding Candy'
 cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_46_Wedding_Candy'
...
def add_texture():
 req=urllib.request.Request(TEXTURE_URL,headers={'User-Agent':'Grilling-A2.7.46/1'})
...
def patch_scripts():
 scripts=BP/'scripts'
 for name in ('a2746_wedding_candy_core.js','a2746_wedding_candy_runtime.js'):
 ...
 add=anchor+"import './a2746_wedding_candy_runtime.js';\n"
 if 'a2746_wedding_candy_runtime.js' in s:raise RuntimeError('A2.7.46 wedding runtime already active')
...
def report():
 write(P/'reports/a2746-wedding-candy.json',{
  'version':'A2.7.46','scope':'port Java Wedding Candy item, Invincible food effect, and September 2026 acquisition event',
  'java_contract':{
...
def main():
 ...
 print('A2.7.46 Wedding Candy complete')
