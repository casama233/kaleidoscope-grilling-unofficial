...
def fetch(path,sha):
 req=urllib.request.Request(UP+path,headers={'User-Agent':'Grilling-A2.7.42/1'})
...
def patch_versions():
 ...
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.42 Sugared Tomato + Pepper Honey BP'),(rm,'Kaleidoscope Grilling A2.7.42 Sugared Tomato + Pepper Honey RP')):
  doc['header']['version']=VERSION;doc['header']['name']=name
  for module in doc.get('modules',[]):module['version']=VERSION
 for dep in bm.get('dependencies',[]):
  if dep.get('uuid')==rm['header']['uuid']:dep['version']=VERSION
 write(BP/'manifest.json',bm);write(RP/'manifest.json',rm)
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.42 Sugared Tomato + Pepper Honey'
 cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_42_Sugared_Tomato_Pepper_Honey'
...
def patch_content():
 ...
 for name,doc in items.items():
  ...
  if path.exists():raise RuntimeError(f'A2.7.42 item already exists: {path}')
  write(path,doc)
 recipes={
 ...
 for name,doc in recipes.items():
  ...
  if path.exists():raise RuntimeError(f'A2.7.42 recipe already exists: {path}')
  write(path,doc)
 atlas=load(RP/'textures/item_texture.json')
 for name,(src,sha) in TEXTURES.items():
  if name in atlas['texture_data']:raise RuntimeError(f'A2.7.42 texture key already exists: {name}')
 ...
 for lang,rows in LANG.items():
  path=RP/'texts'/lang;text=path.read_text(encoding='utf-8');existing=text.splitlines()
...
def patch_effect_registry():
 shutil.copy2(DEV/'a2742_standalone_food_effect_core.js',BP/'scripts/a2732_standalone_food_effect_core.js')

def report():
 write(P/'reports/a2742-sugared-tomato-pepper-honey.json',{
  'version':'A2.7.42','scope':'two missing Java 1.1.1 cold-dessert foods',
  'java_baseline':'breezeth-CN/KaleidoscopeGrilling@9a1acdab27698457bec16c9362678e574895a28c',
  'foods':{
   'sugared_tomato':{'nutrition':6,'saturation_modifier':0.65,'recipe':['c:crops/tomato','minecraft:sugar'],
    'bedrock_recipe':['kaleidoscope_cookery:tomato','minecraft:sugar'],
    'java_common_tomato_tag_breadth_preserved':False,'required_cookery_host_tomato_used':True},
   'pepper_honey':{'nutrition':4,'saturation_modifier':0.25,
    'recipe':['3x kaleidoscope_grilling:sichuan_pepper','minecraft:honey_bottle'],
    'effect':'kaleidoscope_grilling:numb','effect_ticks':1200,'uses_shared_standalone_food_effect_runtime':True}
  },
  'textures':{name:{'git_blob_sha1':sha} for name,(_,sha) in TEXTURES.items()},
  'dedupe':{'new_item_complete_use_listener_created':False,'existing_a2732_shared_effect_runtime_reused':True,'duplicate_cookery_tomato_created':False},
  'maxim_translation_keys_present':True,'maxim_tooltip_runtime_binding_ported':False,
...
def main():
 ...
 print('A2.7.42 Sugared Tomato + Pepper Honey complete')
