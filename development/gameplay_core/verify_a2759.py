...
def main():
 ...
 bm,rm=load(BP/'manifest.json'),load(RP/'manifest.json');assert bm['header']['version']==[2,7,59] and rm['header']['version']==[2,7,59]
 for n in NEW:assert (BP/'scripts'/n).read_bytes()==(DEV/n).read_bytes(),n
 rt=(BP/'scripts/a2759_advancement_inventory_runtime.js').read_text(encoding='utf-8')
 ...
 assert maintext.count("import {awardInventoryAdvancements} from './a2759_advancement_inventory_runtime.js';")==1
 ...
 r=load(P/'reports/a2759-inventory-advancement-parity.json');assert r['version']=='A2.7.59';assert r['cadence_ticks']==20;assert r['reuse']['new_interval_count']==0;assert r['reuse']['new_listener_count']==0;assert r['reuse']['raw_skewer_source']=='Object.keys(RAW_TO_COOKED)';assert r['minecraft_tested'] is False
 compiled=[]
 if a.compiled:
  dist=P/'builds/dist'
 ...
 out.write_text(json.dumps({'version':'A2.7.59','inventory_advancement_parity':True,'new_advancement_count':6,'looking_the_part_inventory_fallback':True,'new_interval_count':0,'new_listener_count':0,'compiled':a.compiled,'compiled_packs':compiled,'minecraft_tested':False,'bds_tested':False},indent=2)+'\n',encoding='utf-8');print(out.read_text())
