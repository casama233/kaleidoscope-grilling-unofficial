...
import {
 CROP_ID,SWEET_POTATO_ID,COMPONENT_ID,AGE_STATE,MAX_AGE,STRAW_HATS,
 ...
 init.blockComponentRegistry.registerCustomComponent(COMPONENT_ID,{
  beforeOnPlayerPlace(event){
   try{
    ...
    if(!sweetPotatoSurvive(below?.typeId??'',lightAt(event.dimension,event.block.location))){event.cancel=true;return}
  ...
  onRandomTick(event){
   try{
    ...
    if(!sweetPotatoSurvive(below?.typeId??'',light)){
     const age=ageOf(block),count=age>=MAX_AGE?matureSweetPotatoCount(0,[Math.random(),Math.random(),Math.random()]):1;
     event.dimension.setBlockType(block.location,'minecraft:air');dropAt(block,SWEET_POTATO_ID,count);return;
    }
...
// A2.7.19 owns the whole Java CropDropHandler ordering.
// A2.7.15/A2.7.17 generated break subscribers are removed by augment_a2719.py.
