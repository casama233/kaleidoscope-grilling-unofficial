...
const shared=new vm.SourceTextModule(fs.readFileSync(new URL('a2714_houttuynia_crop_core.js',root),'utf8'),{context,identifier:'shared'});
const core=new vm.SourceTextModule(fs.readFileSync(new URL('a2715_canola_crop_core.js',root),'utf8'),{context,identifier:'core'});
const rt=new vm.SourceTextModule(fs.readFileSync(new URL('a2715_canola_crop_runtime.js',root),'utf8'),{context,identifier:'runtime'});
...
await core.link(async spec=>spec==='./a2714_houttuynia_crop_core.js'?shared:null);
...
let component;
startup[0]({blockComponentRegistry:{registerCustomComponent(id,obj){assert.equal(id,'kaleidoscope_grilling:canola_crop_logic');component=obj}}});
assert.ok(component);

function makeWorld({ground='minecraft:farmland',moisture=7,light=15,age=0}={}){
 const loc={x:5,y:64,z:8},blocks=new Map(),key=(x,y,z)=>x+','+y+','+z;
 const dimension={
 ...
 for(let dz=-1;dz<=1;dz++)for(let dx=-1;dx<=1;dx++)blocks.set(key(5+dx,63,8+dz),{typeId:ground,permutation:new Perm({moisturized_amount:moisture},ground)});
 const block={typeId:'kaleidoscope_grilling:canola_crop',dimension,location:loc,permutation:new Perm({'kaleidoscope_grilling:age':age},'kaleidoscope_grilling:canola_crop'),setPermutation(p){this.permutation=p}};
 blocks.set(key(5,64,8),block);return{dimension,block};
}
...

{
 ...
 const e={block:w.block,dimension:w.dimension,cancel:false,permutationToPlace:new Perm({},'kaleidoscope_grilling:canola_crop')};
...
{
 const w=makeWorld({ground:'minecraft:dirt',light:15});
 const e={block:w.block,dimension:w.dimension,cancel:false,permutationToPlace:new Perm()};
 component.beforeOnPlayerPlace(e);assert.equal(e.cancel,true);
}
randoms.push(0);
{
 const w=makeWorld({age:3,light:15});component.onRandomTick({block:w.block,dimension:w.dimension});
 assert.equal(w.block.permutation.getState('kaleidoscope_grilling:age'),4);
}
randoms.push(.999);
{
 const w=makeWorld({age:2,light:15});const p=player({boneMeal:new Stack('minecraft:bone_meal',2)});
...
drops.length=0;randoms.push(.999,0);
{
 ...
 assert.equal(drops.length,1);assert.equal(drops[0].stack.typeId,'kaleidoscope_grilling:canola_seeds');assert.equal(drops[0].stack.amount,4);
}

drops.length=0;randoms.push(0,0);
{
 const b={location:{x:1,y:2,z:3},dimension:{spawnItem(stack,where){drops.push({stack,where})}}};
 breaks[0]({block:b,player:player({hat:'minecraft:leather_helmet'}),brokenBlockPermutation:new Perm({},'minecraft:short_grass'),itemStackBeforeBreak:new Stack('minecraft:stick')});
 assert.equal(drops.length,0);
}

console.log(JSON.stringify({passed:6,failed:0,scope:'A2.7.15 canola crop + straw-hat acquisition'}));
