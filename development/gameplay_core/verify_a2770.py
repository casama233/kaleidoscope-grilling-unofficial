"""Creative grouping verification plus unchanged A2.7.69 gameplay regression gates."""
from pathlib import Path
import argparse
import json
import subprocess
import sys
import a2770_creative_catalog as catalog
...
def main():
    ...
    # This release hashes every non-creative payload and every hidden definition.
    # The previous functional gates remain, with only the version parameter changed.
    from verify_a2766 import check_assets
    assets=check_assets()
    maps=prior.source_guards((2,7,70))
    prior.java_contract()
    for script, extra in (
        ('test_a2770_catalog.py',[]),
