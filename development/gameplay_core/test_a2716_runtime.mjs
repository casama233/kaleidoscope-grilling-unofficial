...
const core=new vm.SourceTextModule(fs.readFileSync(new URL('a2716_canola_processing_core.js',root),'utf8'),{context,identifier:'core'});
const rt=new vm.SourceTextModule(fs.readFileSync(new URL('a2716_canola_processing_runtime.js',root),'utf8'),{context,identifier:'runtime'});
...
assert.equal(sent.length,before);
console.log(JSON.stringify({passed:6,failed:0,scope:'A2.7.16 canola millstone registration'}));
