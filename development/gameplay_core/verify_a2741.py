...
def main():
 ...
 assert bm['header']['name']=='Kaleidoscope Grilling A2.7.41 Sugared Tomato + Pepper Honey BP'

 expected={
  'sugared_tomato':(6,0.65),'pepper_honey':(4,0.25)
 }
 for name,(nutrition,sat) in expected.items():
 ...
 assert core.read_bytes()==(DEV/'a2741_standalone_food_effect_core.js').read_bytes()
 ...
 assert "FX_KEY='kaleidoscope_grilling:a21_fx'" in core.read_text(encoding='utf-8')
 assert "effect:'numb'" in core.read_text(encoding='utf-8') and 'PEPPER_HONEY_NUMB_TICKS=1200' in core.read_text(encoding='utf-8')
 ...
 for lang in ('en_US.lang','zh_CN.lang','zh_TW.lang'):
  ...
  for key in (
   'item.kaleidoscope_grilling:sugared_tomato.name',
   'tooltip.kaleidoscope_grilling.sugared_tomato.maxim',
   'item.kaleidoscope_grilling:pepper_honey.name',
 ...
 report=load(P/'reports/a2741-sugared-tomato-pepper-honey.json')
 ...
 result={'version':'A2.7.41','sugared_tomato':True,'pepper_honey':True,'pepper_honey_numb_ticks':1200,
