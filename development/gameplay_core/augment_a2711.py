...
def fetch(path,sha):
 req=urllib.request.Request(UP+path,headers={'User-Agent':'Grilling-A2.7.11/1'})
...
def replace_once(s,old,new,label):
 if old not in s:raise RuntimeError('A2.7.11 patch anchor missing: '+label)
 return s.replace(old,new,1)

def patch_versions():
 bm,rm=load(BP/'manifest.json'),load(RP/'manifest.json')
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.11 Mantou Chopping BP'),(rm,'Kaleidoscope Grilling A2.7.11 Mantou Chopping RP')):
  doc['header']['version']=VERSION;doc['header']['name']=name
  for m in doc.get('modules',[]):m['version']=VERSION
 for dep in bm.get('dependencies',[]):
  if dep.get('uuid')==rm['header']['uuid']:dep['version']=VERSION
 write(BP/'manifest.json',bm);write(RP/'manifest.json',rm)
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.11 Mantou Chopping';cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_11_Mantou_Chopping';write(P/'config.json',cfg)
...
def patch_runtime():
 shutil.copy2(DEV/'a2711_mantou_chopping_core.js',BP/'scripts/a2711_mantou_chopping_core.js')
 shutil.copy2(DEV/'a2711_mantou_chopping_runtime.js',BP/'scripts/a2711_mantou_chopping_runtime.js')
 path=BP/'scripts/main.js';s=path.read_text(encoding='utf-8')
 anchor="import './a2710_chicken_acquisition_runtime.js';"
 s=replace_once(s,anchor,anchor+"\nimport './a2711_mantou_chopping_runtime.js';",'runtime import')
 path.write_text(s,encoding='utf-8')

def report():
 write(P/'reports/a2711-parity.json',{
  'version':'A2.7.11',
  ...
  'cookery_bedrock':{
   ...
   'initial_ping_owner':'A2.7.8 runtime; A2.7.11 only adds an api_ready listener to avoid duplicate pings'
  },
  'downstream':{
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,10]:raise RuntimeError('A2.7.11 must augment verified A2.7.10')
 patch_versions();patch_item();patch_runtime();report();print('A2.7.11 mantou chopping complete')
