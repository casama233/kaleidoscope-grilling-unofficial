...
IDS=('kaleidoscope_grilling:sugared_tomato','kaleidoscope_grilling:pepper_honey')
LANG={
 'en_US.lang':{
...
def fetch(path,sha):
 req=urllib.request.Request(UP+path,headers={'User-Agent':'Grilling-A2.7.49/1'})
...
def replace_once(s,old,new,label):
 if s.count(old)!=1:raise RuntimeError(f'A2.7.49 patch anchor drift ({label}): {s.count(old)}')
 return s.replace(old,new,1)

def patch_versions():
 bm,rm=load(BP/'manifest.json'),load(RP/'manifest.json')
 for doc,name in (
  (bm,'Kaleidoscope Grilling A2.7.49 Sugared Tomato + Pepper Honey BP'),
  (rm,'Kaleidoscope Grilling A2.7.49 Sugared Tomato + Pepper Honey RP'),
 ):
 ...
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.49 Sugared Tomato + Pepper Honey'
 cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_49_Sugared_Tomato_Pepper_Honey'
...
def patch_content():
 ...
 for name,doc in items.items():
  ...
  if path.exists():raise RuntimeError(f'A2.7.49 item already exists: {path}')
  write(path,doc)
 recipes={
 ...
 for name,doc in recipes.items():
  ...
  if path.exists():raise RuntimeError(f'A2.7.49 recipe already exists: {path}')
  write(path,doc)

 atlas=load(RP/'textures/item_texture.json')
 for name,(src,sha) in TEXTURES.items():
  if name in atlas['texture_data']:raise RuntimeError(f'A2.7.49 texture key already exists: {name}')
 ...
 if group is None:raise RuntimeError('A2.7.49 main catalog group missing')
 rows=group.setdefault('items',[])
 for item_id in IDS:
  if item_id in rows:raise RuntimeError(f'A2.7.49 duplicate item catalog id: {item_id}')
  rows.append(item_id)
 write(catalog_path,catalog)

 for lang,entries in LANG.items():
  path=RP/'texts'/lang;text=path.read_text(encoding='utf-8');rows=text.splitlines()
  for key,value in entries.items():
   if any(row.startswith(key+'=') for row in rows):raise RuntimeError(f'{lang}: duplicate localization key {key}')
   if text and not text.endswith('\n'):text+='\n'
   text+=key+'='+value+'\n'
  path.write_text(text,encoding='utf-8')

def patch_effect_registry():
 source=DEV/'a2749_sugared_tomato_pepper_honey_core.js'
 shutil.copy2(source,BP/'scripts/a2749_sugared_tomato_pepper_honey_core.js')
...
"""
 import_add=import_anchor+"import {pepperHoneyEffectRow} from './a2749_sugared_tomato_pepper_honey_core.js';\n"
 s=replace_once(s,import_anchor,import_add,'effect import')
 tail=""" Object.freeze({
...
"""
 tail_add=tail.replace(" })\n]);"," }),\n pepperHoneyEffectRow()\n]);")
 s=replace_once(s,tail,tail_add,'effect row')
 p.write_text(s,encoding='utf-8')

def report():
 write(P/'reports/a2749-sugared-tomato-pepper-honey.json',{
  'version':'A2.7.49',
  'scope':'rebase and finish the existing Sugared Tomato + Pepper Honey Java-parity slice on published A2.7.48',
  'java_baseline':'breezeth-CN/KaleidoscopeGrilling@9a1acdab27698457bec16c9362678e574895a28c',
  'foods':{
   'sugared_tomato':{'nutrition':6,'saturation_modifier':0.65,
    'java_recipe':['c:crops/tomato','minecraft:sugar'],
    'bedrock_recipe':['kaleidoscope_cookery:tomato','minecraft:sugar'],
    'required_cookery_host_tomato_used':True},
   'pepper_honey':{'nutrition':4,'saturation_modifier':0.25,
    'recipe':['3x kaleidoscope_grilling:sichuan_pepper','minecraft:honey_bottle'],
    'effect':'numb','effect_ticks':1200,'uses_shared_standalone_food_effect_runtime':True}
  },
  'dedupe':{
   'existing_pr_46_source_reused':True,
   'new_item_complete_use_listener_created':False,
   'existing_a2732_shared_effect_runtime_reused':True,
   'wedding_candy_effect_row_preserved':True,
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,48]:
  raise RuntimeError('A2.7.49 must augment published A2.7.48')
 patch_content();patch_effect_registry();patch_versions();report()
 print('A2.7.49 Sugared Tomato + Pepper Honey complete')
