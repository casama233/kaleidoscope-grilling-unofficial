...
def main():
 ...
 assert bm['header']['name']=='Kaleidoscope Grilling A2.7.58 P1 Completion BP'

 assert (BP/'scripts'/CORE).read_bytes()==(DEV/CORE).read_bytes()
 assert (BP/'scripts'/RUNTIME).read_bytes()==(DEV/RUNTIME).read_bytes()
 assert (BP/'blocks'/BLOCK).read_bytes()==(DEV/'a2758_pepper_leaves_fruiting_bridge.block.json').read_bytes()
 assert (BP/'features'/FEATURE).read_bytes()==(DEV/'a2758_pepper_tree_worldgen.feature.json').read_bytes()

 main_text=(BP/'scripts/main.js').read_text(encoding='utf-8')
 assert main_text.count("import './a2758_pepper_worldgen_fruiting_runtime.js';")==1
 assert main_text.index("import './a2748_pepper_tree_runtime.js';")<main_text.index("import './a2758_pepper_worldgen_fruiting_runtime.js';")
 ...
 report=load(P/'reports/a2758-p1-completion.json')
 ...
 result={
  'version':'A2.7.58','p1_actionable_complete':True,
