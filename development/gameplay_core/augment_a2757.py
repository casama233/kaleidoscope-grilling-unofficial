...
def patch_versions():
 ...
 for doc,name in (
  (bm,'Kaleidoscope Grilling A2.7.57 P0 Official API Hardening BP'),
  (rm,'Kaleidoscope Grilling A2.7.57 P0 Official API Hardening RP'),
 ):
 ...
 cfg['name']='Kaleidoscope Grilling A2.7.57 P0 Official API Hardening'
 cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_57_P0_Official_API_Hardening'
 write(P/'config.json',cfg)

def report():
 write(P/'reports/a2757-p0-official-api-hardening.json',{
  'version':'A2.7.57',
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,56]:
  raise RuntimeError('A2.7.57 must augment published A2.7.56')
 patch_adapter();patch_versions();report()
 print('A2.7.57 P0 Official API Hardening complete')
