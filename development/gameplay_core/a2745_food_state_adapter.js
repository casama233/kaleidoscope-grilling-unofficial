...
export function setHotFood(stack,ticks){
 ...
 try{
  ...
  // Keep the existing order: make the stack custom before writing the stable item dynamic property.
  stack.setLore(lore);
...
export function applyFoodMetadata(stack,{seasoning=[],hotTicks=0}={}){
 if(!stack)return stack;
 setFoodSeasonings(stack,seasoning);
 setHotFood(stack,hotTicks);
 return stack;
}
