...
VERSION=[2,7,59]
NEW=('a2759_advancement_inventory_core.js','a2759_advancement_inventory_runtime.js')
LANG={
'en_US.lang':[
...
def versions():
 ...
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.59 Inventory Advancement Parity BP'),(rm,'Kaleidoscope Grilling A2.7.59 Inventory Advancement Parity RP')):
  doc['header']['version']=VERSION;doc['header']['name']=name
  for m in doc.get('modules',[]):m['version']=VERSION
 for dep in bm.get('dependencies',[]):
  if dep.get('uuid')==rm['header']['uuid']:dep['version']=VERSION
 write(BP/'manifest.json',bm);write(RP/'manifest.json',rm)
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.59 Inventory Advancement Parity';cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_59_Inventory_Advancement_Parity';write(P/'config.json',cfg)
...
def scripts():
 ...
 if s.count(anchor)!=1:raise RuntimeError('A2.7.59 import anchor drift')
 s=s.replace(anchor,anchor+"import {awardInventoryAdvancements} from './a2759_advancement_inventory_runtime.js';\n",1)
 anchor=" for(const p of world.getAllPlayers()){try{\n"
 if s.count(anchor)!=1:raise RuntimeError('A2.7.59 player loop anchor drift')
...
def report():
 write(P/'reports/a2759-inventory-advancement-parity.json',{
 'version':'A2.7.59','new_advancements':['human_fireworks','better_write_it_down','a_handful_of_canola','strength_makes_oil','sweet_potato','nether_taste'],
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,58]:raise RuntimeError('A2.7.59 must augment published A2.7.58')
 scripts();lang();versions();report();print('A2.7.59 inventory advancement parity complete')
