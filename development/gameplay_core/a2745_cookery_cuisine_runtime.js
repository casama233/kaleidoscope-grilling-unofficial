...
import {
 playerInventory,getMainHand,setMainHand,isCreative
} from './a2735_player_io.js';
import {
 readCookeryOilPot
} from './a2734_cookery_oil_pot_adapter.js';
import {
 SEASONING_MAX_USES,SEASONING_USES_KEY
...
function clearCuisineAt(dimensionId,location){
 try{
  world.setDynamicProperty(cuisineStateKeyAt(dimensionId,location.x,location.y,location.z),undefined);
  return true;
 }catch{return false}
}
...
function primitiveProps(stack){
 const out={};let ids=[];
 try{ids=stack?.getDynamicPropertyIds()??[]}catch{}
...
function descriptor(stack){
 ...
 let lore=[],name='',damage=null;
 try{lore=stack.getLore()}catch{}
 try{name=stack.nameTag??''}catch{}
...
function snapshotInventory(player){
 ...
 const rows=[];
 for(let i=0;i<c.size;i++)rows.push(descriptor(c.getItem(i)));
 return rows;
}

function foodLike(stack){
...
function rewriteGain(player,gain,plan){
 ...
 const custom=current.clone();custom.amount=gain.count;
 applyFoodMetadata(custom,plan);
 const remain=current.amount-gain.count;
 if(remain>0){
  current.amount=remain;c.setItem(gain.slot,current);giveCustom(player,custom);
...
function typedHeldOil(stack){
 ...
 if(stack.typeId===COOKERY_FILLED_OIL_POT_ID){
  ...
  return oilTypeFromHeld(stack.typeId,state?.type??'');
 }
...
function setUses(stack,n){
 try{stack.setDynamicProperty(SEASONING_USES_KEY,Math.max(0,Math.min(SEASONING_MAX_USES,n|0)))}catch{}
 return stack;
}
...
function applySeasoning(player,block){
 ...
 if(plan.mutate){
  if(plan.replaceEmpty)setMainHand(player,new ItemStack(EMPTY_SEASONING_ID,1));
   ...
   try{
    ...
    lore.unshift('§7Uses: '+(SEASONING_MAX_USES-plan.nextUses)+'/'+SEASONING_MAX_USES);
    next.setLore(lore);
   }catch{}
 ...
 message(player,'§a已向 Cookery 鍋具加入特製調料');
 return true;
}
...
function finishHostInteraction(player,dimension,location,kind,beforeInventory,beforeHasOil,candidateOil,stateBefore){
 ...
 const afterInventory=snapshotInventory(player);
 ...
 const effective={
  ...stateBefore,
  oilType:kind==='pot'?(stateBefore.oilType||(beforeHasOil?'default':'')):stateBefore.oilType
 };
 const plan=metadataPlan(kind,effective);
 let decorated=0;
 for(const gain of gains)if(rewriteGain(player,gain,plan))decorated++;

 if(decorated>0){
  if(kind==='pot')clearCuisineAt(dimension.id,location);
  else if(block?.typeId===COOKERY_STOCKPOT_ID)
   writeCuisineState(block,{...stateBefore,lastOutputTick:system.currentTick});
 }
}
...
world.beforeEvents.playerInteractWithBlock.subscribe(event=>{
 try{
  ...
  if(main?.typeId===SEASONING_ID){
   ...
   system.run(()=>{
    const block=dimension.getBlock(location);
    if(block&&stationKind(block.typeId)===kind)applySeasoning(player,block);
   });
   return;
  }

  const dimension=event.block.dimension,location={...event.block.location};
  const stateBefore=readCuisineState(event.block);
  const beforeInventory=snapshotInventory(player);
  const beforeHasOil=kind==='pot'?hostHasOil(event.block):false;
  const candidateOil=kind==='pot'?typedHeldOil(main):'';
  system.run(()=>finishHostInteraction(
   player,dimension,location,kind,beforeInventory,beforeHasOil,candidateOil,stateBefore
  ));
 }catch{}
});
...
world.beforeEvents.playerBreakBlock.subscribe(event=>{
 try{
  ...
  const dimensionId=event.block.dimension.id,location={...event.block.location};
  system.run(()=>clearCuisineAt(dimensionId,location));
 }catch{}
});
...
world.beforeEvents.explosion.subscribe(event=>{
 try{
  ...
  for(const block of event.getImpactedBlocks()){
   if(stationKind(block?.typeId))rows.push({dimensionId:block.dimension.id,location:{...block.location}});
  }
