...
def patch_versions():
 ...
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.27 Localization BP'),(rm,'Kaleidoscope Grilling A2.7.27 Localization RP')):
 ...
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.27 Localization';cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_27_Localization';write_json(P/'config.json',cfg)
...
def patch_languages():
 ...
 if order!=tw_order or order!=en_order or set(cn)!=set(tw) or set(cn)!=set(en):
  raise RuntimeError('language key sets/order drift before A2.7.27')
...
def report(count):
 write_json(P/'reports/a2727-localization.json',{
  'version':'A2.7.27','scope':'language file normalization only','language_keys':count,
...
def main():
 ...
 patch_versions();count=patch_languages();report(count);print('A2.7.27 localization complete')
