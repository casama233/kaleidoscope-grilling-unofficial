...
BLOCKS={
 'a2748_pepper_log.block.json':'pepper_log.json',
 'a2748_pepper_leaves.block.json':'pepper_leaves.json',
 'a2748_pepper_sapling.block.json':'pepper_sapling.json',
}
LOOT={
 'a2748_pepper_log.loot.json':'pepper_log.json',
 'a2748_pepper_sapling.loot.json':'pepper_sapling.json',
 'a2748_pepper_leaves_fallback.loot.json':'pepper_leaves_fallback.json',
}
...
def patch_versions():
 ...
 for doc,name in ((bm,'Kaleidoscope Grilling A2.7.48 Pepper Tree Lifecycle BP'),(rm,'Kaleidoscope Grilling A2.7.48 Pepper Tree Lifecycle RP')):
  doc['header']['version']=VERSION;doc['header']['name']=name
  for module in doc.get('modules',[]):module['version']=VERSION
 for dep in bm.get('dependencies',[]):
  if dep.get('uuid')==rm['header']['uuid']:dep['version']=VERSION
 write(BP/'manifest.json',bm);write(RP/'manifest.json',rm)
 cfg=load(P/'config.json');cfg['name']='Kaleidoscope Grilling A2.7.48 Pepper Tree Lifecycle'
 cfg['compiler']['plugins'][0][1]['packName']='Kaleidoscope_Grilling_A2_7_48_Pepper_Tree_Lifecycle'
...
def patch_scripts():
 ...
 anchor="import './a2747_wedding_candy_runtime.js';\n"
 add=anchor+"import './a2748_pepper_tree_runtime.js';\n"
 if s.count(anchor)!=1:raise RuntimeError('A2.7.48 main import anchor drift')
 if 'a2748_pepper_tree_runtime.js' in s:raise RuntimeError('A2.7.48 runtime already active')
...
def patch_behavior_content():
 ...
 shutil.copy2(DEV/'a2748_oak_planks_from_pepper_log.recipe.json',BP/'recipes/oak_planks_from_pepper_log.json')
...
def report():
 write(P/'reports/a2748-pepper-tree-lifecycle.json',{
  'version':'A2.7.48',
...
def main():
 if load(BP/'manifest.json')['header']['version']!=[2,7,47]:raise RuntimeError('A2.7.48 must augment published A2.7.47')
 patch_scripts();patch_behavior_content();patch_resource_content();patch_lang();patch_versions();report()
 print('A2.7.48 Pepper Tree lifecycle complete')
