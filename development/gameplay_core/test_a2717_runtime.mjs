...
const core=new vm.SourceTextModule(fs.readFileSync(new URL('a2717_onion_crop_core.js',root),'utf8'),{context,identifier:'core'});
const rt=new vm.SourceTextModule(fs.readFileSync(new URL('a2717_onion_crop_runtime.js',root),'utf8'),{context,identifier:'runtime'});
...
await core.link(async spec=>spec==='./a2715_canola_crop_core.js'?shared15:null);
...
let component;
startup[0]({blockComponentRegistry:{registerCustomComponent(id,obj){assert.equal(id,'kaleidoscope_grilling:onion_crop_logic');component=obj}}});
assert.ok(component);

function makeWorld({ground='minecraft:farmland',moisture=7,light=15,age=0}={}){
 const loc={x:6,y:64,z:9},blocks=new Map(),key=(x,y,z)=>x+','+y+','+z;
 const dimension={
 ...
 for(let dz=-1;dz<=1;dz++)for(let dx=-1;dx<=1;dx++)blocks.set(key(6+dx,63,9+dz),{typeId:ground,permutation:new Perm({moisturized_amount:moisture},ground)});
 const block={typeId:'kaleidoscope_grilling:onion_crop',dimension,location:loc,permutation:new Perm({'kaleidoscope_grilling:age':age},'kaleidoscope_grilling:onion_crop'),setPermutation(p){this.permutation=p}};
 blocks.set(key(6,64,9),block);return{dimension,block};
}
...
{
 const w=makeWorld();const e={block:w.block,dimension:w.dimension,cancel:false,permutationToPlace:new Perm({},'kaleidoscope_grilling:onion_crop')};
...
randoms.push(0);
{
 const w=makeWorld({age:4});component.onRandomTick({block:w.block,dimension:w.dimension});
 assert.equal(w.block.permutation.getState('kaleidoscope_grilling:age'),5);
}
randoms.push(.999);
{
...
drops.length=0;randoms.push(.999,0);
{
 ...
 assert.equal(drops.length,1);assert.equal(drops[0].stack.typeId,'kaleidoscope_grilling:onion');assert.equal(drops[0].stack.amount,4);
}
...
console.log(JSON.stringify({passed:6,failed:0,scope:'A2.7.17 onion crop + straw-hat acquisition'}));
