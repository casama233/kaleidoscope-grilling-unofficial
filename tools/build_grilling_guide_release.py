...
def main():
    ...
    if tuple(int(x) for x in rm["header"]["version"]) != version:
        raise RuntimeError("guide BP/RP version mismatch")

    output = args.output
    if output is None:
        label = "_".join(map(str, version))
        output = ROOT / "artifacts" / "candidates" / f"Kaleidoscope_Grilling_Guide_A{label}_Canonical.mcaddon"
    ...
    with zipfile.ZipFile(output) as zf:
        if zf.testzip() is not None:
            raise RuntimeError("guide candidate ZIP integrity failed")
        ...
        if tuple(bp_inside["header"]["version"]) != version or tuple(rp_inside["header"]["version"]) != version:
            raise RuntimeError("packaged guide manifest version mismatch")
        if "behavior_pack/scripts/main.js" not in zf.namelist():
            raise RuntimeError("packaged guide Script API entry missing")
        payload = zf.read("behavior_pack/scripts/payload.js").decode("utf-8")
        if "export const GUIDE_PAYLOAD=" not in payload or '"version":"0.2.0"' not in payload:
            raise RuntimeError("packaged Guide A2 payload missing")
